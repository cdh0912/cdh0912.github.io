    ...
    $.timeCube = function(el, options){
        ...
        base.init = function(){
        	...
        	$(base.options.data).each(function(index, d){
	        	...
	        	base.markerDivs[index].attr('id', d.startDate.getFullYear() + '-' + getMonth(d.startDate) + '-' + getDay(d.startDate)); 
	        	base.markerDivs[index].css("left", base.getNavPosition(d.startDate) + "px");
	        	base.markerDivs[index].append("<div class='starIcon'>"
								        	+ "<p class='starNumber'>34</p></div>"
								        	+ "<p class='markerDate'>" + (d.startDate.getMonth() + 1) + ' / ' + d.startDate.getDate() +"</p>");
			...
			// FIRST BUTTON
			$("#first-link").bind((base.touch ? 'touchstart' : 'mousedown touchstart'), function(evt) {
				$("#first-link").addClass("disabled");
				
				firstPage();
				bc = 0;
				base.snapToIndex(bc);
				
			 	$("#last-link").removeClass("disabled");	
			});

			
			// LAST BUTTON
			$("#last-link").bind((base.touch ? 'touchstart' : 'mousedown touchstart'), function(evt) {
				$("#last-link").addClass("disabled");
				
				var markersCount = base.markerDivs.length;
				bc = markersCount - 1;
				base.snapToIndex(bc);				
			...
			if(base.options.nextButton != undefined){
				$(base.options.nextButton).bind((base.touch ? 'touchstart' : 'mousedown touchstart'), function(evt) {
					...
					if(base.currentIndex < (base.options.data.length - 1)){
						...
						bc = base.currentIndex + 1;
						base.snapToIndex(bc);
					}
					var startMonth = startDate.getFullYear()*12 + startDate.getMonth()+1;
					var MonthOfMarkerArray = $(".line .active").attr('id').split('-');
					var markerMonth = Number(MonthOfMarkerArray[0])*12 + Number(MonthOfMarkerArray[1]);
					
					if(startMonth+1 < markerMonth) {
						...
						var startMonth2 = Number(json[len].startDate.getFullYear())*12 + Number(json[len].startDate.getMonth())+1;

						if(startMonth2 == markerMonth || startMonth2-1 == markerMonth) {
							$("#last-link").addClass("disabled");									
						}
					}
					return false;				
			...
			if(base.options.previousButton != undefined){
				$(base.options.previousButton).bind((base.touch ? 'touchstart' : 'mousedown touchstart'), function(evt) {
					...
					if(startMonth > markerMonth) {	

						previousPage();

						$("#last-link").removeClass("disabled");
						
						var startMonth2 = Number(json[0].startDate.getFullYear())*12 + Number(json[0].startDate.getMonth())+1;
		...
		base.snapToIndex = function(index){
			...
			//날짜 표시//////////////////////////////////////////////////////////////////////////////////////////////////////////			
			nowDate = new Date(base.markerDivs[index].attr('id'));
			var year = '<font size="4">'+nowDate.getFullYear()+'</font> ';
			var month = (nowDate.getMonth()+1)+'<font size="4">월</font> ';
			var day = nowDate.getDate()+'<font size="4">일</font> ';
			
			$(".textInDivider").fadeOut(200, function() {
				$(this).html(year + month + day).fadeIn(200);
			});

			$("#nowPage").val(base.markerDivs[index].attr('id'));
		...
		base.getNavPosition = function(date){
			...
			var timelineStartTime = base.startDate.getDate();
			var timelineEndTime = base.endDate.getDate();
...
function removeUselessMarker() {
	var arr = [-8, -9, -1, +0, +3, +4];
	var fd = firstDate.getMonth() + monthPlus;
	for(var i =0; i<arr.length; i++) {
		...
		$(".line > [id*=-"+ tmp +"-]").css('display', 'none');	//1
	}
//	
//	$(".line > [id*=-"+ (fd - 8) +"-]").css('display', 'none');	//1
//	$(".line > [id*=-"+ (fd - 9) +"-]").css('display', 'none');	//2
// 	$(".line > [id*=-"+ (fd - 1) +"-]").css('display', 'none');	//8
// 	$(".line > [id*=-"+ (fd	   ) +"-]").css('display', 'none');	//9
// 	$(".line > [id*=-"+ (fd + 3) +"-]").css('display', 'none');	//12
// 	$(".line > [id*=-"+ (fd + 4) +"-]").css('display', 'none');	//13
}
...
function nextPage() {
	monthPlus += 2;
	$("#timeline").empty();
	startDate = new Date(firstDate.getFullYear(), firstDate.getMonth() + monthPlus, 1);
	endDate = new Date(firstDate.getFullYear(), firstDate.getMonth() + 2 + monthPlus, 1);
 	...
 	$(".legend .label:nth-child(1)").remove();
	$(".legend .label:nth-child(3)").remove();
...
function lastPage(){
	...
	var len = json.length - 1;
	var lastDate = json[len].startDate;
	
	monthPlus = lastDate.getMonth() - firstDate.getMonth();
	if(lastDate.getYear() > firstDate.getYear()) {
		monthPlus += 12;
	}
